...
public class OutgoingTCPReplicator extends AbstractTCPReplicator {
		...
		start.addListener(f -> {
			if (f.isSuccess()) {
				...
				if (logger.isDebugEnabled()) {
					logger.debug("Failed to synchronize with {} on {}", remoteId, ctx.channel()
						.remoteAddress(), f.cause());
				}
				ctx.close();
			}
		});
	...
	protected int validateExchangeHeader(ChannelHandlerContext ctx, long incomingExchangeId, UUID incomingId,
		...
		if (incomingExchangeId != exchangeId) {
			logger.warn("The TCP cluster exchange with " + ctx.channel()
				.remoteAddress() + " responded with an invalid id");
			return -1;
		}
		if (remoteId != null) {
			if (!remoteId.equals(incomingId)) {
				logger.warn("The TCP cluster exchange with " + ctx.channel()
					.remoteAddress() + " responded with an unexpected id " + incomingId + " not " + remoteId);
				return -1;
			}
