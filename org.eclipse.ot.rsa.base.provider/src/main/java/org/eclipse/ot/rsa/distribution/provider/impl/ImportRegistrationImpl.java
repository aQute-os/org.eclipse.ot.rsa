...
import static org.eclipse.ot.rsa.distribution.provider.impl.RegistrationState.CLOSED;
import static org.eclipse.ot.rsa.distribution.provider.impl.RegistrationState.ERROR;
import static org.eclipse.ot.rsa.distribution.provider.impl.RegistrationState.OPEN;
import static org.eclipse.ot.rsa.distribution.provider.impl.RegistrationState.PRE_INIT;
...
import org.eclipse.ot.rsa.distribution.config.ImportedServiceConfig;
import org.eclipse.ot.rsa.distribution.provider.client.ClientConnectionManager;
import org.eclipse.ot.rsa.distribution.provider.proxy.ClientServiceFactory;
...
public class ImportRegistrationImpl implements ImportRegistration {
	...
	private final String					_frameworkUUID;
	...
	public ImportRegistrationImpl(EndpointDescription endpoint, Framework targetFramework,
		...
		_methodMappings = Arrays.stream(_config.org_eclipse_ot_rsa_imported__methods())
				.map(s -> s.split("="))
				.collect(Collectors.toMap(s -> Integer.valueOf(s[0]), s -> s[1]));

		List<URI> uris = Arrays.stream(_config.org_eclipse_ot_rsa_imported_net())
				.map(URI::create)
				.collect(toList());
		...
		try {
			reg = _hostBundleContext.registerService(
					...
					SerializationType.of(_config.org_eclipse_ot_rsa_imported_serialization())
						.getFactory(),
		...
		// We must sync and check here as we've escaped into the wild by
	...
	private long getServiceTimeout() {
		long serviceTimeout = _config.org_eclipse_ot_rsa_imported__timeout();
	...
	public void close() {
		...
		// We must remove before unregistering so that the service reference (if
		// valid) is still
	...
	public boolean update(EndpointDescription endpoint) {
		synchronized (this) {
			...
			if (!Arrays.deepEquals(_config.org_eclipse_ot_rsa_imported__methods(),
					tmpConfig.org_eclipse_ot_rsa_imported__methods())) {
