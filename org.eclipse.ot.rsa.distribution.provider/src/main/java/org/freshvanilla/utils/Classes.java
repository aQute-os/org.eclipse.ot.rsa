...
public class Classes {
	...
	public static <T> T parseAs(Object o, Class<T> clazz) {
		...
		if (Number.class.isAssignableFrom(clazz2)) {
			...
			if (clazz2 == Byte.class)
				return (T) (Byte) Byte.parseByte(asString);
			if (clazz2 == Short.class)
				return (T) (Short) Short.parseShort(asString);
			if (clazz2 == Integer.class)
				return (T) (Integer) Integer.parseInt(asString);
			if (clazz2 == Long.class)
				return (T) (Long) Long.parseLong(asString);
			if (clazz2 == Float.class)
				return (T) (Float) Float.parseFloat(asString);
			if (clazz2 == Double.class)
				return (T) (Double) Double.parseDouble(asString);
			if (clazz2 == BigInteger.class)
				return (T) new BigInteger(asString);
			if (clazz2 == BigDecimal.class)
				return (T) new BigDecimal(asString);
		}
