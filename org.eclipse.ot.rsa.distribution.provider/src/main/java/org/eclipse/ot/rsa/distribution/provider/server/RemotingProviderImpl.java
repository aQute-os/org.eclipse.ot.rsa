...
class RemotingProviderImpl implements RemotingProvider {
	...
	public RemotingProviderImpl(ProtocolScheme p, ServerRequestHandler handler, Channel channel, ChannelGroup group) {
		this.secure = p.getProtocol()
			.isSecure();
		this.scheme = p.getProtocol()
			.getUriScheme();
		...
		if (addressesToAdvertise.isEmpty()) {
			...
			endpoints = Collections.unmodifiableList(addressesToAdvertise.entrySet()
				.stream()
		...
		if (address.getAddress()
			...
			try {
				...
				throw new IllegalArgumentException(
					"Unable to determine the real addresses to advertise for this RemotingProvider", e);
			}
