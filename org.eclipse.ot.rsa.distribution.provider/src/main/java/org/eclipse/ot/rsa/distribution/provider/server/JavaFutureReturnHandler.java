...
class JavaFutureReturnHandler extends BasicReturnHandler {
	...
	public JavaFutureReturnHandler(UUID serviceId, Serializer serializer, Future<?> completeFuture,
			...
			return executor.submit(() -> {
				try {
					...
					sendReturn(channel, callId, true,
						((java.util.concurrent.Future<?>) returnValue).get(30, java.util.concurrent.TimeUnit.SECONDS));
