...
public class ServerConnectionManager {
	...
	public ServerConnectionManager(TransportConfig config, ParemusNettyTLS tls, ByteBufAllocator allocator,
		...
		configuredTransports = Arrays.stream(protocols)
			...
			.filter(p -> {
				...
				LOG.warn(
					"The server transport {} is not permitted because it is insecure and insecure transports are not enabled.",
					p.getProtocol());
				return false;
			})
		...
		if (configuredTransports.isEmpty() && protocols.length > 0) {
			LOG.error(
				"There are no server transports available for this provider. Please check the configuration. DistributionConfig was "
					+ Arrays.toString(config.server_protocols()));
	...
	private RemotingProviderImpl createProviderFor(ProtocolScheme p, InetSocketAddress defaultBindAddress) {
		...
		switch (p.getProtocol()) {
			...
			case TCP :
				...
				c = c.andThen(ch -> {
					// Incoming
					ch.pipeline()
						.addLast(new VersionCheckingLengthFieldBasedFrameDecoder());
				});
				break;
			default :
		...
		try {
			...
			LOG.warn("Interruped while configuring the transport {} with configuration {}", p.getProtocol(),
				p.getConfigurationString());
