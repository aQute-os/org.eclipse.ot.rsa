...
public class ParemusNettyTLSImpl implements NettyTLS {
	...
	private KeyManagerFactory setupKeyManager(TransportTLSConfig config, Provider jceProvider, Provider jsseProvider)
		...
		try (InputStream is = Files.newInputStream(new File(config.keystore_location()).toPath())) {
			keyStore.load(is, config._keystore_password()
				.toCharArray());
		}

		String keystoreKeyPassword = config._keystore_key_password();

		kmf.init(keyStore, keystoreKeyPassword.isEmpty() ? config._keystore_password()
			.toCharArray() : keystoreKeyPassword.toCharArray());
		return kmf;
	}

	private TrustManagerFactory setupTrustManager(TransportTLSConfig config, Provider jceProvider,
		Provider jsseProvider) throws NoSuchAlgorithmException, KeyStoreException, IOException, CertificateException {
		...
		try (InputStream is = Files.newInputStream(new File(config.truststore_location()).toPath())) {
			trustStore.load(is, config._truststore_password()
				.toCharArray());
		}
	...
	public DTLSClientHandler getDTLSClientHandler() {
		...
		return new org.eclipse.ot.rsa.tls.netty.provider.dtls.jsse.ParemusClientDTLSHandler(
	...
	public DTLSHandler getDTLSServerHandler() {
		...
		return new org.eclipse.ot.rsa.tls.netty.provider.dtls.jsse.ParemusServerDTLSHandler(
			new JdkDtlsEngineAdapter(engine));
	}
